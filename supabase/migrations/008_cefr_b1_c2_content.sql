...
begin
  foreach code in array array['B1','B2','C1','C2'] loop
    select * into level_row from public.levels where levels.code = code limit 1;
    ...
    select l.id into lesson_id from public.lessons l where l.unit_id = unit_id and l.title = lesson_title limit 1;
    ...
    if not exists (select 1 from public.lesson_blocks b where b.lesson_id = lesson_id) then
    ...
    if not exists (select 1 from public.exercises e where e.lesson_id = lesson_id) then
